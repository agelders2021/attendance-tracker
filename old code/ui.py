...
class TrainingTrackerApp:
    ...
    def _generate_participation_excel(self, excel_folder: str):
        """Generate the Participation.xlsx file on exit.
        ...
        try:
            ...
            filepath = os.path.join(excel_folder, "Participation.xlsx")
            backup_path = os.path.join(excel_folder, "Participation-back.xlsx")
            ...
            for col, session in enumerate(sessions, start=2):
                ...
                print(f"DEBUG: Excel - Col {col}: date='{date_str}', loc='{location}', type='{session_type}', header='{header_text}'")
                ...
                if session_type in fills:
                    ...
                    print(f"DEBUG: Excel - Applied {session_type} color to header col {col}")
                else:
                    print(f"DEBUG: Excel - Unknown session type '{session_type}', no color applied")
            ...
            for col in range(2, len(sessions) + 2):
            ...
            print(f"DEBUG: Excel - Error generating Excel: {e}")
    ...
    def _send_attendance_emails_to_all(self):
        ...
        try:
            ...
            if not sessions_6mo:
                print("DEBUG: Email - No sessions in last 6 months for email")
                return
            ...
            print(f"DEBUG: Email - Total weekend sessions in 6mo period: {total_weekend_sessions}")
            ...
            print(f"DEBUG: Email - Encryption type: {smtp_encryption}")
            print(f"DEBUG: Email - Username: {smtp_username}")
            print(f"DEBUG: Email - Sender: {sender_email}")
            
            server = None
            try:
                if smtp_encryption == "SSL":
                    print("DEBUG: Email - Using SSL connection")
                    context = ssl.create_default_context()
                    server = smtplib.SMTP_SSL(smtp_server, smtp_port, context=context)
                else:
                    print("DEBUG: Email - Using plain SMTP connection")
                    server = smtplib.SMTP(smtp_server, smtp_port)
                    server.set_debuglevel(1)  # Enable SMTP debug output
                    
                    if smtp_encryption == "TLS":
                        print("DEBUG: Email - Starting TLS")
                        tls_result = server.starttls()
                        print(f"DEBUG: Email - STARTTLS result: {tls_result}")
                    elif smtp_encryption == "STARTTLS":
                        print("DEBUG: Email - Starting STARTTLS")
                        tls_result = server.starttls()
                        print(f"DEBUG: Email - STARTTLS result: {tls_result}")
                        
                print("DEBUG: Email - Attempting login...")
                login_result = server.login(smtp_username, sender_password)
                print(f"DEBUG: Email - Login result: {login_result}")
                
            except smtplib.SMTPAuthenticationError as e:
                print(f"DEBUG: Email - Authentication failed: code={e.smtp_code}, msg={e.smtp_error}")
                if server:
                    server.quit()
                return
            except smtplib.SMTPException as e:
                print(f"DEBUG: Email - SMTP error: {type(e).__name__}: {e}")
                ...
                print(f"DEBUG: Email - Connection error: {type(e).__name__}: {e}")
                if server:
                    try:
                        server.quit()
                    except:
            ...
            for member in members:
                ...
                print(f"DEBUG: Email - Processing member: {member_name} <{email}>")
                ...
                if pdf_path:
                    try:
                        ...
                        # Send email
                        print(f"DEBUG: Email - Sending email to {email}...")
                        send_result = server.sendmail(sender_email, email, msg.as_string())
                        print(f"DEBUG: Email - Send result: {send_result}")
                        emails_sent += 1
                        print(f"DEBUG: Email - Successfully sent to {email}")
                        
                    except smtplib.SMTPRecipientsRefused as e:
                        print(f"DEBUG: Email - Recipients refused for {email}: {e.recipients}")
                    except smtplib.SMTPException as e:
                        print(f"DEBUG: Email - SMTP error sending to {email}: {type(e).__name__}: {e}")
                        if hasattr(e, 'smtp_code'):
                            print(f"DEBUG: Email - SMTP code: {e.smtp_code}")
                    except Exception as e:
                        print(f"DEBUG: Email - Error sending to {email}: {type(e).__name__}: {e}")
                    
                    # Clean up temp file
                    try:
                        os.remove(pdf_path)
                        print(f"DEBUG: Email - Cleaned up temp file: {pdf_path}")
                    except Exception as e:
                        print(f"DEBUG: Email - Could not remove temp file: {e}")
                else:
            ...
            try:
                server.quit()
                print("DEBUG: Email - SMTP connection closed")
            except Exception as e:
                print(f"DEBUG: Email - Error closing SMTP: {e}")
                
            print(f"DEBUG: Email - Total emails sent: {emails_sent}")
            # ===END DEBUG===
    ...
    def _generate_member_attendance_pdf(self, member: dict, sessions_3mo: list, sessions_6mo: list, total_weekend: int) -> str:
        ...
        try:
            ...
            print(f"DEBUG: PDF - Member attended {weekend_attended} weekend sessions in 6mo")
            ...
            print(f"DEBUG: PDF - Error generating PDF for {member.get('last_name', '')}: {e}")
            return None
    ...
    def _on_save_member(self):
        ...
        member_data = {
            ...
            'certifications': self.vars.get_certification_dates_dict(),
        }
